...
const TelegramVoiceSetup: FC<{
    ...
    const hasAuth = () => {
        const hasBot = !!(form.values.botToken || config?.hasBotToken);
        const hasSession = !!(form.values.sessionString || config?.hasSession);
        return hasBot || hasSession;
    };

    return (
        <Flex direction="column" gap="sm">
            <Text size="sm" c="dimmed">
                Stream the Airstation HLS feed live into Telegram group or channel voice chats using py-tgcalls.
                Provide either a Bot API token or a Pyrogram session string. The Bot API token is used to create an
                MTProto bot session; if Telegram rejects bot streaming, switch to a user session string generated by
                the helper script.
            </Text>
            ...
            <PasswordInput
                label="Bot API token"
                placeholder={config?.hasBotToken ? "Saved. Leave empty to keep it." : "Paste Bot API token"}
                description="From @BotFather. Either this or a session string is required."
                disabled={loading}
            ...
            <PasswordInput
                ...
                description="Run python tools/telegram_login.py to generate a user session."
                disabled={loading}
            ...
            <Group mt="sm" justify="space-between">
                <Button
                    ...
                    disabled={!form.values.apiID || (!form.values.apiHash && !config?.hasAPIHash) || !hasAuth()}
                >
                    Test credentials
                </Button>
